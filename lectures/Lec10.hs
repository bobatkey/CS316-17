...
{- It might be tempting to think that nearly every type that takes a
   parameter is a Functor. Here is an example of a type that is not a
   Functor: the type of functions from a type to itself: -}
...
{-      PART II : FOLDABLE
   ...
   of of turning individual 'a's into 'b's. What else can we do with a
...
{- Looking at these three functions, we can see that they only differ in
   ...
   'add'-like thing a "monoid". And we make a type class to represent
   it: -}
...
{- So a type 'm' is a 'Monoid' if there is a 'base' element of 'm', and
   an operation 'op' that takes two 'm's and returns an 'm'. In the
   Haskell standard library, the 'Monoid' typeclass is already
   defined, but uses the names 'mempty' and 'mappend' for 'base' and
   'op'. We think these names are ugly.

   Just as for 'Functor' above, 'base' and 'op' should obey some laws:
     ...
     1. For all x, y, z in 'm', it should be the case that:
        ...
        *not* say that the order of the arguments to 'op' doesn't
        matter, see below.)

     2. for all x,   base `op` x == x

        This is usually called "left unit". It states that 'base'
        really acts like a 'zero' when combined on the left.

     3. for all x,   x `op` base == x

        This is usually called "right unit". It states that 'base'
...
{- Using the 'Monoid' type class, we can now write a generic function
   ...
   the list, using 'op'. Other names for this function include
...
{- Comparing to the three functions we wrote above, 'foldList' uses
   'base' whenever they use the "zero"-like thing, and 'op' whenever
   they use the "add"-like thing. We can now recover the three
   functions above simply by giving different types to 'foldList',
   which has the effect of telling Haskell which instance of 'Monoid'
   we want to use: -}
...
{- Don't be scared by the fact that all these functions have the same
   ...
   all the elements. We can also do this for 'Tree's, using 'base' for
   the 'Leaf's, and 'op' twice to add up the results of the left tree,
   the data, and the right subtree. Note that the order in which we do
   this -- (left `op` x) `op` right, left `op` (data `op` right) --
...
{- We can also "add up" all the elements stored in a "Maybe"
   ...
   need to use 'op', but we use 'base' for the "Nothing" case. -}
...
{- However, in some cases it might be more efficient to define a special
   ...
   define 'True' and '&&' as the 'base' and 'op' of the 'Monoid'
   instance for 'Bool'. But this is not the only choice.
   ...
   But GHC complains:
      ...
      Lec10.hs:317:10-20: Duplicate instance declarations: …
            instance Monoid Bool
               -- Defined at .../Lec10.hs:317:10
            instance Monoid Bool
               -- Defined at .../Lec10.hs:468:10
